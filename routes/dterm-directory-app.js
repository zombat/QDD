...
router.get(`/:context`, (req, res) => {
	mongoClient.get().db(process.env.SYSTEM_VARIABLES_DATABASE).collection(`global-configuration`).findOne({ _id: `core-server-settings`},(err, coreServerSettings) => {
		...
		if(req.query.hasOwnProperty(`support`) && req.query.support.match(/true/i)){
					...
					if(documents.length === 0){
						...
						assert.equal(null, err);			
							if(parseInt(req.query.skip) > 0){
								itemArray.push([`...Previous Results`,`${coreServerSettings.serverProtocol}://${coreServerSettings.serverHostname}/dtd/${req.params.context}?skip=${req.query.skip - resultLimit}`]);
							}
						documents.forEach((document) =>{
							itemArray.push([`${document.firstName} ${document.lastName}`,`${coreServerSettings.serverProtocol}://${coreServerSettings.serverHostname}/dtd/${req.params.context}?id=${document._id}`]);
						});
						if(documentCount > resultLimit){
							itemArray.push([`More Results...`,`${coreServerSettings.serverProtocol}://${coreServerSettings.serverHostname}/dtd/${req.params.context}?skip=${parseInt(req.query.skip) + resultLimit}`]);
						}
			...
			necXML.generateDtermIPInput(`Search Directory`, `${coreServerSettings.serverProtocol}://${coreServerSettings.serverHostname}/dtd/${req.params.context}`, `Search`, `Text`, `searchString`, `5`, [[`Back`,`SoftKey:Back`],[`Delete`,`SoftKey:BackSpace`]], (dtermIPInput) => {
				res.writeHead(200, { 'Content-Type': `text/html` });
					...
					if(documents.length === 0){
						...
						let itemArray = [];
							if(parseInt(skipCount) > 0){
								itemArray.push([`...Previous Results`,`${coreServerSettings.serverProtocol}://${coreServerSettings.serverHostname}/dtd/${req.params.context}?searchString=${req.query.searchString}:skip=${skipCount - resultLimit}`]);
							}
							Object.keys(documents).forEach((key) =>{
								itemArray.push([`${documents[key].firstName} ${documents[key].lastName}`,`${coreServerSettings.serverProtocol}://${coreServerSettings.serverHostname}/dtd/${req.params.context}?id=${documents[key]._id}`]);
							});
							if(documentCount > resultLimit){
								itemArray.push([`More Results...`,`${coreServerSettings.serverProtocol}://${coreServerSettings.serverHostname}/dtd/${req.params.context}?searchString=${req.query.searchString}:skip=${skipCount + resultLimit}`]);
							}
				...
				mongoClient.get().db(process.env.DIRECTORY_DATABASE).collection(process.env.DIRECTORY_COLLECTION).find({ _id:  objectID }).toArray((err, document) => {
					...
					if(document[0].contactMethods){
						document[0].contactMethods.forEach((contactMethod) => {
							individualResultScreen += `<ListItem name="${contactMethod.contactMethodName}"><URI param="use">${coreServerSettings.serverProtocol}://${coreServerSettings.serverHostname}/dtd/${req.params.context}</URI><Parameters><Param key="index" value="${document[0].contactMethods.indexOf(contactMethod)}" /><Param key="id" value="${document[0]._id}"/></Parameters></ListItem>\n`;
							});
						}
			...
			necXML.generateDtermIPList(`Corporate Directory`,[[`Browse Directory`,`${coreServerSettings.serverProtocol}://${coreServerSettings.serverHostname}/dtd/${req.params.context}?browse=true`],[`Search Directory`,`${coreServerSettings.serverProtocol}://${coreServerSettings.serverHostname}/dtd/${req.params.context}?search=true`],[`Support Information`,`${coreServerSettings.serverHostname}/dtd/${req.params.context}?support=true`]],[[`Back`, `SoftKey:Back`]], (dtermIPList) =>{
				res.writeHead(200, { 'Content-Type': `text/html` });
				res.end(dtermIPList);
			});	
